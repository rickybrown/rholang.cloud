...
class DockerManager {
  ...
  static reset(image) {
    ...
    const name = 'rchain-local-v6-version-' + currentVersion.replace(/\./g, '')
  ...
  static runWithInputWithoutSocket(data, response) {
    // first temporarily store file
    const id = uuid()
    const dir = '/tmp/rnode/' + id
    const filename = 'input.rho'
      ...
      fs.mkdir(dir, 0o777, function () {
        fs.writeFile(path, contents, 'utf8', function () {
          ...
          const streamo = new StringWritable(chunk => {
            ...
            //socket.emit('output.append', [chunk, type])
            returnValue[type].push(chunk)
            if (type === 'storageContents') {
              //response.send(returnValue)
              return
            }
            //returnValue.push([type, chunk])
